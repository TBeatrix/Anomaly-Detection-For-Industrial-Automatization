...
class LoadImagesForClustering:
    # YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`
    def __init__(self, path, anomalies, img_size=640, stride=32, auto=True):
        # Modified for more source path
        self.files = []
        self.nf = 0
        self.img_size = img_size
        self.stride = stride
        self.isAnomaly = anomalies
     
        
        # for dir in path:
          
        #     if dir.split("/")[3] == 'normal' or dir.split("/")[3] == 'normal2':
        #         isAnomaly = False
        #     else:
        #         isAnomaly = True
        #     p = str(Path(dir).resolve())  # os-agnostic absolute path
        #     if '*' in p:
        ...
        #    # self.isAnomaly.extend([isAnomaly] * ni)
           
        #     self.files.append(path)
          
        #     self.nf = self.nf +ni  # number of files
        ...
        self.auto = auto
        self.files = path
        paired_list = list(zip(self.files, self.isAnomaly))
        print("anomalies", len(self.isAnomaly))
        # Step 2: Shuffle the paired list
        random.shuffle(paired_list)

        # Step 3: Unzip the shuffled list back into two lists
        self.files, self.isAnomaly = zip(*paired_list)

        # Convert back to lists (zip returns tuples)
        self.files = list(self.files)
        self.isAnomaly = list(self.isAnomaly)
    ...
    def __next__(self):
        ...
        isAnomaly = self.isAnomaly[self.count]
        ...
        return path, img, img0, isAnomaly, s
